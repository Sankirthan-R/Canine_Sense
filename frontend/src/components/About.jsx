...
const About = () => {
  return (
    <section
      id="about"
      className="py-32 relative z-10 bg-[var(--color-dark)] shadow-[0_-20px_50px_rgba(0,0,0,0.8)]"
    >
        ...
        <div className="text-center mb-20">
          ...
          <Reveal delay={0.12}>
            <p className="text-gray-400 max-w-2xl mx-auto text-lg">
              We combine two specialized neural networks into a single, seamless pipeline. Discover how our
              multimodal approach sets a new standard for animal-computer interaction.
            </p>
          </Reveal>
        </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {features.map((feature, i) => (
            <motion.div
              ...
              className="glass p-8 rounded-3xl relative group overflow-hidden cursor-default"
            >
              {/* Hover glow */}
              <div className="absolute top-0 right-0 w-32 h-32 bg-[var(--color-gold)] opacity-0 group-hover:opacity-10 rounded-bl-full transition-opacity duration-500 blur-2xl" />

              <div className="mb-6 p-4 rounded-2xl bg-white/5 w-fit border border-white/10 group-hover:border-[var(--color-gold)]/50 transition-colors duration-300">
                {feature.icon}
              </div>

              <h3 className="text-xl font-bold mb-4 text-white group-hover:text-[var(--color-gold)] transition-colors duration-300">
                {feature.title}
              </h3>
